...
## Dataset and loader
parser.add_argument('--velocity_ckpt', type=str, default='./pretrained/vm_ckpt.pt')
...
## Training
parser.add_argument('--train_straight_network', type=eval, default=False, choices=[True, False])
...
# Logging
if args.logging:
    log_dir = get_new_log_dir(args.log_root, prefix='STRAIGHTENED_D%s_' % (args.dataset), postfix='_' + args.tag if args.tag is not None else '')
    ...
    shutil.copy('./train_straight.py', log_dir) 
    shutil.copy('./test_straight.py', log_dir) 
...
for i in range(args.num_modules):
    velocity_models.append(DirectionNet(args=velocity_ckpt['args']).to(args.device))
    velocity_models[i].load_state_dict(velocity_ckpt['state_dict'])
model = StraightPFN(velocity_models, args).to(args.device)
