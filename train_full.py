...
## Dataset and loader
parser.add_argument('--straight_ckpt', type=str, default='./pretrained_straight/cvm_ckpt.pt')
...
## Training
parser.add_argument('--train_straight_network', type=eval, default=True, choices=[True, False])
...
# Logging
if args.logging:
    log_dir = get_new_log_dir(args.log_root, prefix='FULL_D%s_' % (args.dataset), postfix='_' + args.tag if args.tag is not None else '')
    ...
    shutil.copy('./train_full.py', log_dir) 
    shutil.copy('./test_full.py', log_dir) 
...
train_dset = PairedPatchDataset(
    ...
    patch_ratio=1.2,
    train_straight_network=args.train_straight_network
)
...
logger.info('Number of velocity modules in SC: {}'.format(straight_ckpt['args'].num_modules))
straight_model = StraightPFN(args=straight_ckpt['args']).to(args.device)
straight_model.load_state_dict(straight_ckpt['state_dict'])
model = FullPFN(straight_model, args).to(args.device)
