...
class PairedPatchDataset(Dataset):
    ...
    def make_patches(self):
        for dataset in tqdm(self.datasets, desc='MakePatch'):
            for data in tqdm(dataset):
                ...
                pat_std = data['noise_std']   
                for i in range(pat_noisy.size(0)):
                    self.patches.append((pat_noisy[i], pat_clean[i], pat_std[i], ))
    ...
    def __getitem__(self, idx):
        if self.on_the_fly:
            ...
            pat_noisy, pat_clean, seed_pts = make_patches_for_pcl_pair(
                pcl_data['pcl_noisy'],
                pcl_data['pcl_clean'],
                patch_size=self.patch_size,
                num_patches=1,
            ...
            data = {
                ...
                'seed_pnts': self.patches[idx][2].clone(),
                'pcl_std': self.patches[idx][3].clone(),
            }
