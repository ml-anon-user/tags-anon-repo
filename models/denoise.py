...
from datasets.pcl_with_normals import *
from datasets.patch_with_normals import *
...
class DenoiseNet(pl.LightningModule):
    ...
    def train_dataloader(self):
        ...
        train_dset = PairedPatchWithNormalsDataset(
            datasets=[
                PointCloudWithNormalsDataset(
    ...
    def val_dataloader(self):
        # Datasets and loaders
        val_dset = PointCloudWithNormalsDataset(
                        root=self.args.dataset_root,
                        dataset=self.args.dataset,
                        split='test',
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        pcl_normals = train_batch['pcl_clean_normals']
        pcl_std = train_batch['pcl_std']

        # Forward
        if self.args.loss_type == "NN":
            loss = self.get_supervised_loss_nn(pcl_noisy=pcl_noisy, pcl_clean=pcl_clean, pcl_seeds=pcl_seeds, pcl_std=pcl_std, stdze_loss=self.args.stdze_loss)  
        elif self.args.loss_type == "NN_no_stitching":
            loss = self.get_supervised_loss_nn_no_weighting(pcl_noisy=pcl_noisy, pcl_clean=pcl_clean, pcl_seeds=pcl_seeds, pcl_std=pcl_std, stdze_loss=self.args.stdze_loss)
    ...
    def get_supervised_loss_nn(self, pcl_noisy, pcl_clean, pcl_seeds, pcl_std, stdze_loss=False):
        """
        ...
        for i in range(self.num_modules):
            ...
            if stdze_loss and self.noise_decay != 1:
                dist = dist / (prev_std - curr_std).unsqueeze(1)
            losses[i] = (seed_weights * dist).sum(dim=-1).mean(dim=-1)
            
        return losses.sum() #, target, scores, noise_vecs

    def get_supervised_loss_nn_no_weighting(self, pcl_noisy, pcl_clean, pcl_seeds, pcl_std, stdze_loss=False):
        """
        Denoising score matching.
        Args:
            pcl_noisy:  Noisy point clouds, (B, N, 3).
