...
from models.denoise import DirectionNet
...
class StraightPFN(nn.Module):
    ...
    def __init__(self, velocity_nets=None, args=None):
        ...
        if velocity_nets is not None:
            ...
            for i in range(self.num_modules):
                self.velocity_nets.append(DirectionNet(args=velocity_ckpt['args']).to(args.device))
    ...
    def denoise_langevin_dynamics(self, pcl_noisy):
        ...
        with torch.no_grad():
            ...
            tot_its = 3 #int(self.tot_its / 2)

            for it in range(tot_its):
                # Trajectories
                ...
                for mod in range(self.num_modules):
                    ...
                    pcl_next = pcl_next + (1 / tot_its) * (1 / self.num_modules) * pred_dir # 0.75
